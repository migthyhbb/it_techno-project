...
export async function POST(req: Request) {
  try {
    ...
    // 1. VALIDASI KEAMANAN: Pastikan yang mengakses ini BENAR-BENAR ADMIN
    ...
    // 2. KUNCI MASTER (SERVICE ROLE): Bypass RLS dengan aman HANYA di Server
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
    const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY!;
    const supabaseAdmin = createClient(supabaseUrl, serviceRoleKey);

    // 3. Update Status Pengiriman Limbah
    ...
    // 4. LOGIKA SUNTIK TOKEN OTOMATIS (Aman di dalam Server)
    if (
      ...
      // Ambil saldo lama dari database
