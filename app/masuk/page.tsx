...
export default function MasukPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { data: adminRow } = await supabase.from("admin_profiles").select("user_id").eq("user_id", userId).maybeSingle();
      if (adminRow) {
        setStatus("submitted");
      ...
      // Cek apakah dia agen/mitra
      const { data: mitraRow } = await supabase.from("mitra_profiles").select("user_id").eq("user_id", userId).maybeSingle();
      if (mitraRow) {
        setStatus("submitted");
      ...
      // Cek apakah dia industri
      const { data: industriRow } = await supabase.from("industri_profiles").select("user_id").eq("user_id", userId).maybeSingle();
