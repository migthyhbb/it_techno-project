...
export default function DaftarIndustriPage() {
  ...
  // Fetch Provinsi saat komponen di-mount
  useEffect(() => {
  ...
  const handleProvinsiChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    update("kota", ""); // Reset kota saat provinsi berubah
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Step 0 — kirim kode verifikasi ke email
    if (step === 0) {
      setStatus("loading");
      try {
        await sendOtp();
    ...
    // Step 3 — detail profil + validasi format + simpan
    ...
    try {
      ...
      if (form.foto_npwp) {
        ...
        // Asumsi kamu membuat bucket baru bernama 'industri_documents'
      ...
      const { error: profileError } = await supabase.from("industri_profiles").upsert({
        ...
        npwp: form.npwp,
        alamat: alamatLengkap, // Format: "Detail, Kota, Provinsi"
        telepon: form.telepon,
        foto_npwp_url: fotoUrl, // Kolom baru di tabel industri_profiles
      }, {
        onConflict: 'user_id'
      });
      ...
      if (err instanceof Error && err.message === "no-session") {
        ...
        setError("Email ini sudah terdaftar sebagai industri. Silakan masuk.");
      } else if (err && typeof err === "object" && "message" in err) {
        setError(`Error Supabase: ${(err as { message: string }).message}`);
      } else {
      ...
      {status === "submitted" ? (
        <div className="text-center py-4">
          ...
          <p className="text-ink/55 text-sm">
            Akun dan profil industri kamu sudah tersimpan. Mengalihkan ke dashboard...
          </p>
        </div>
      ) : (
        <>
                ...
                {step === 1 && (
                  <>
                    ...
                    <button
                      ...
                      className="text-xs text-green hover:underline disabled:text-ink/35 disabled:no-underline mb-2"
                    >
                      {resendCooldown > 0
                ...
                {step === 3 && (
                  <>
                    <FormField
                      ...
                      placeholder="PT / CV ..."
                      error={fieldErrors.nama_perusahaan}
                      required
                      autoFocus
                    />
                    ...
                    <div className="mb-4 text-left">
                      ...
                      <input
                        ...
                        className="block w-full text-sm text-ink/80 border border-ink/20 rounded-md p-2 focus:outline-none focus:ring-1 focus:ring-green file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-green/10 file:text-green hover:file:bg-green/20"
                        required
                      />
                    ...
                    <div className="mb-4 text-left">
                      ...
                      <select
                        className={`block w-full text-sm text-ink/80 border ${fieldErrors.provinsi ? 'border-red-500' : 'border-ink/20'} rounded-md p-2.5 focus:outline-none focus:ring-1 focus:ring-green bg-white`}
                      ...
                      {fieldErrors.provinsi && (
                        <p className="text-xs text-red-600 mt-1.5">{fieldErrors.provinsi}</p>
                      )}
                    </div>
                    ...
                    <div className="mb-4 text-left">
                      ...
                      <select
                        className={`block w-full text-sm text-ink/80 border ${fieldErrors.kota ? 'border-red-500' : 'border-ink/20'} rounded-md p-2.5 focus:outline-none focus:ring-1 focus:ring-green bg-white disabled:bg-ink/5 disabled:cursor-not-allowed`}
                        onChange={(e) => update("kota", e.target.options[e.target.selectedIndex].text)}
                      ...
                      {fieldErrors.kota && (
                        <p className="text-xs text-red-600 mt-1.5">{fieldErrors.kota}</p>
                      )}
                    </div>

                    {/* Input Detail Alamat (Otomatis Kapital) */}
                    <FormField
                      label="Detail Alamat"
                      type="text"
                      value={form.detail_alamat}
                      onChange={(e) => update("detail_alamat", e.target.value.toUpperCase())}
                      placeholder="Jalan, RT/RW, no. gedung"
                      error={fieldErrors.detail_alamat}
                      required
                    />
